				       ...
				       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_KI_COEFF_CURRENT:
		...
		break;
	default:
		pr_debug_ratelimited("unsupported property %d\n", psp);
		rc = -EINVAL;
		break;
	}
...
static void fg_battery_soc_smooth_tracking(struct fg_gen4_chip *chip)
{
	...
	calculate_delta_time(&last_change_time, &time_since_last_change_sec);
	trace_printk("entry:smooth_batt_soc%d\n", fg->param.smooth_batt_soc);
	...
	trace_printk("FG_ok: soc:%d, last_soc:%d, raw_soc:%d, soc_changed:%d, batt_ma:%d, smooth_low_batt_soc:%d, smooth_soc: %d\n",
				fg->param.batt_soc, last_batt_soc,
